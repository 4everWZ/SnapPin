...
| Iterate UI/action logic with tests | `snappin_tests` covers overlay selection-hole logic, overlay show-order flags, overlay reopen at a new rect after hide, compact artifact toolbar width budget, compact annotate toolbar tool presence/selection, annotate client-width independence from toolbar width, `BG` toolbar state, `Clr` toolbar presence, mark composed-copy pixel smoke paths for Rect, Ellipse, Line, Arrow, Serial, Mosaic, Blur, Highlighter, Spotlight, Polyline, Watermark, Magnifier, Pencil, and Text, Text background fill output changes, Text background color-cycle output changes, Mosaic/Blur wheel-strength output changes, Highlighter wheel opacity/strength output changes, Spotlight wheel dim-strength pixel changes, Watermark direct text and wheel opacity/strength output changes, Magnifier wheel zoom internal-pixel changes, Polyline node-drag edit output change, Polyline segment double-click node insertion, Polyline selected-node deletion, Eraser deletion restoring source pixels, Eraser path-segment removal preserving remaining Polyline pixels, layout helpers, mosaic/OCR helpers, focused-pin OCR context, pin context-menu label/content-kind gating, pin context-menu action-to-command mapping, OCR result-window text selection, OCR result-window repeat-copy callback, OCR result-window empty-result `Copy` disabled state, and tray notification safety; `ui_capture_smoke` launches the real app, opens capture through the hidden main window, drag-selects a region, verifies compact toolbar visibility, creates a real pin through toolbar `Pin`, closes the pin window, repeats capture selection, opens the child mark window through toolbar `Mark`, verifies overlay session state, and closes mark | Partially covered | Tests still do not verify actual clipboard contents, OCR runtime recognition, visible tray notifications, advanced/deferred mark tools, or full manual mouse workflows |
| Improve mark tools that are far behind reference app | Added/covered `Ellipse`, `Serial`, `Mosaic`, `Blur`, `Eraser`, `Highlighter`, `Spotlight`, editable multi-click `Polyline`, basic rectangular `Watermark`, and basic rectangular `Magnifier` baseline tools; Polyline now supports basic node drag, whole move, segment double-click node insertion, and selected-node deletion; Text supports a basic background fill toggle and preset background color cycling; Highlighter supports wheel opacity/strength; Spotlight supports wheel dim-strength; Watermark supports direct text entry and wheel opacity/strength; Magnifier supports wheel zoom; Serial supports direct numeric entry and `+`/`-` value adjustment; Mosaic/Blur support mouse-wheel strength; Eraser supports basic path-segment removal | Partially covered | Missing richer polyline editing/manual verification, smart erase, advanced watermark presets/batch/centered workflow, advanced magnifier modes/configuration, advanced blur/highlighter/spotlight modes, Auto Mosaic/synced mosaic operations, advanced serial formats/settings, text arrows and richer text background color configuration, full reference app partial/reverse erasing |
| Improve OCR behavior that is far behind reference app | OCR now covers toolbar whole-artifact OCR, `Shift+OCR` selected-region OCR, and focused image pin source paths with clipboard copy, strict `source` selection, auto-selected selectable result window plus repeat-copy action that is disabled for empty text, tray feedback, and declared source/region action parameters | Partially covered | Missing pin OCR text overlay, formula/QR/barcode/table recognition, language/model configuration, advanced result management, and runtime OCR manual verification |
| Release-ready / formal version | Build, CTest, diff check, and process UI smoke pass for current slice | Not covered | Manual end-to-end capture/mark/OCR/pin verification and missing parity decisions are still blockers |
...
| "Mark tools are far behind" | `src/ui/AnnotateWindow.h`, `src/ui/AnnotateWindow.cpp`, `src/ui/AnnotationEffects.h`, `src/ui/AnnotateLayout.h` | Added/covered Ellipse, Serial, Mosaic, Blur, Eraser, Highlighter, Spotlight, editable multi-click Polyline, basic rectangular Watermark, basic rectangular Magnifier, compact BG/Clr text-background toolbar controls, annotate client-width guard, serial direct numeric entry/adjustment, Highlighter wheel opacity/strength, Spotlight wheel dim-strength, Watermark direct text and wheel opacity/strength, Magnifier wheel zoom, Mosaic/Blur wheel strength, layout helpers, composed-copy pixel smoke paths for baseline drawing tools, Polyline node-edit, segment insertion, and selected-node deletion coverage, Eraser deletion source-restore coverage, and Eraser path-segment coverage | Advanced/deferred reference app mark tools and full manual mouse workflows are not yet automated |
| "OCR is far behind" | `src/app/ActionDispatcher.cpp`, `src/app/OcrRegion.h`, `src/app/OcrToolbarMode.h`, `src/app/OcrResultEvent.h`, `src/app/AppMain.cpp`, `src/app/ActionRegistry.cpp`, `src/app/PinManager.*`, `src/ui/OcrResultWindow.*`, `src/ui/PinWindow.*`, `src/app/TrayIcon.*` | OCR source paths cover active artifact, `Shift+OCR` selected region, and focused image pin; `ocr.start` source/region parameters are declared; result text is copied and shown auto-selected in a selectable window; result-window repeat copy is callback-tested and disabled for empty result text; tray feedback exists | Pin OCR overlays and advanced recognizers are still missing |
| Tests gate | `cmake --build "build/MSVC v143 x64 (vcvars64 + Ninja)-Release" --config Release --target all --`; `ctest --test-dir "build/MSVC v143 x64 (vcvars64 + Ninja)-Release" --output-on-failure -C Release`; `no_reference_terms` tracked-file scan; `no_reference_terms_history` fetched-history scan | Passed for current slice | Unit tests do not cover every release requirement |
...
| Process UI smoke gate | CTest `ui_capture_smoke` starts `snappin.exe`, sends the capture command through `SnapPinHiddenWindow`, verifies `SnapPinOverlay` is visible with valid bounds, drag-selects a region, verifies compact `SnapPinToolbar` bounds, creates a real `SnapPinPinWindow` through toolbar `Pin`, closes the pin, repeats capture selection, opens child `SnapPinAnnotateWindow` through toolbar `Mark`, verifies overlay remains visible behind mark, closes mark, then closes the process | Passed for current slice | Smoke covers baseline capture selection, toolbar visibility, artifact-to-pin creation, and mark session entry only; it does not replace manual mark drawing, pin interaction, or OCR workflow verification |
...
- Verified:
  ...
  - `no_reference_terms` and `no_reference_terms_history` CTest gates for forbidden reference terms in tracked content, tracked file names, commit messages, ref names, and reachable history.
  - Process UI smoke: `ui_capture_smoke` launched `snappin.exe`, opened the capture overlay through the hidden main window command path, verified visible overlay bounds, drag-selected a region, verified compact toolbar bounds, created a real pin window through toolbar `Pin`, closed the pin, repeated capture selection, opened the child mark window through toolbar `Mark`, verified overlay session state, closed mark, and closed the process.
- Not yet verified:
  - Manual UI mark loop.
...
- Relevant tests / harnesses / benchmarks / commands:
  - `snappin_tests` covers core rect defaults, overlay selection-hole logic, overlay show-order flags, overlay reopen at a new rect after hide, compact artifact toolbar width budget, AnnotateWindow compact tool button presence/selection for Ellipse, Serial, Mosaic, Blur, Eraser, Highlighter, Spotlight, Polyline, Watermark, Magnifier, BG, and Clr, annotate client-width independence from toolbar width, mark composed-copy pixel changes for Rect, Ellipse, Line, Arrow, Serial, Mosaic, Blur, Highlighter, Spotlight, Polyline, Watermark, Magnifier, Pencil, and Text, Text background fill output changes, Text background color-cycle output changes, Mosaic/Blur wheel-strength output changes, Highlighter wheel opacity/strength output changes, Spotlight wheel dim-strength pixel changes, Watermark direct text and wheel opacity/strength output changes, Magnifier wheel zoom internal-pixel changes, Polyline node editing changing composed output, Polyline segment double-click node insertion preserving endpoints, Polyline selected-node deletion preserving remaining segments, Eraser deletion restoring source pixels, Eraser path-segment removal preserving remaining Polyline pixels, serial value adjustment clamping, direct serial numeric entry changing composed output, annotate toolbar min-width and oversized work-area clamp calculations, mosaic block-size calculation, OCR source selection, OCR toolbar mode selection, OCR region-to-bitmap crop mapping, `ocr.start` focused-pin context and source/region parameter registration, pin context-menu label/content-kind gating, pin context-menu action-to-command mapping, OCR text progress-event UTF-8 roundtrip, OCR result window selectable text behavior, OCR result-window auto-selection, OCR result-window repeat-copy callback, OCR result-window empty-result `Copy` disabled state, image-pin-only OCR command eligibility, and TrayIcon notification safety when uninitialized.
...
- 2026-05-15 / mark text background color iteration - added a BG Color preset cycle for Text annotations with composed-output regression coverage; text arrows and richer text background configuration remain unimplemented.
- 2026-05-15 / mark toolbar layout iteration - decoupled annotate client width from toolbar preferred width so narrow captures do not gain blank side bands, and switched mark toolbar buttons to compact labels while preserving baseline tool coverage.
- 2026-05-15 / pin context-menu model iteration - centralized pin context-menu labels by content kind and covered image/text/LaTeX OCR gating in `snappin_tests`.
- 2026-05-15 / pin context-menu dispatch iteration - covered context-menu action to focused-pin command mapping and made right-click dispatch reuse the tested model.
- 2026-05-15 / OCR source selection iteration - extracted strict OCR source resolution for auto, active artifact, focused pin, no-source, and invalid-source cases.
- 2026-05-15 / artifact toolbar compact layout iteration - replaced the fixed 366px artifact toolbar width with a tested compact 291px layout budget.
- 2026-05-15 / OCR toolbar mode iteration - made toolbar `OCR` invoke active-artifact OCR by default and reserved selected-region OCR for `Shift+OCR`.
- 2026-05-15 / capture overlay reopen iteration - reset overlay selection/hover state, position, and mask region before showing a new capture session; added an overlay reopen window regression and process-level capture drag-select/toolbar/pin/mark smoke.
