...
- Basic OCR over active artifact bitmap, selected OCR region, or focused image pin, copied to clipboard, shown in an auto-selected selectable result window with a repeat-copy action, and paired with tray success/failure feedback; `ocr.start` declares optional `source`, `x`, `y`, `w`, and `h` action parameters.
...
2. Manually smoke-test current mark/OCR baselines, including Blur, Watermark, Magnifier, the selectable OCR result window, auto-selection, and the result-window `Copy` action, then record results.
