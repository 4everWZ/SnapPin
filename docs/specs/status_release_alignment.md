...
| Tests gate | `cmake --build "build/MSVC v143 x64 (vcvars64 + Ninja)-Release" --config Release --target all --`; `ctest --test-dir "build/MSVC v143 x64 (vcvars64 + Ninja)-Release" --output-on-failure -C Release`; `no_reference_terms` tracked-file scan | Passed for current slice | Unit tests do not cover every release requirement |
...
- Verified:
  ...
  - `no_reference_terms` CTest gate for tracked direct reference URL references.
