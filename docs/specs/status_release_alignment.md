...
| Process UI smoke gate | CTest `ui_capture_smoke` starts `snappin.exe`, sends the capture command through `SnapPinHiddenWindow`, verifies `SnapPinOverlay` is visible with valid bounds, drag-selects a region, verifies compact `SnapPinToolbar` bounds, closes the artifact UI, then closes the process | Passed for current slice | Smoke covers baseline capture selection and toolbar visibility only; it does not replace manual mark, pin, or OCR workflow verification |
...
- Verified:
  ...
  - Process UI smoke: `ui_capture_smoke` launched `snappin.exe`, opened the capture overlay through the hidden main window command path, verified visible overlay bounds, drag-selected a region, verified compact toolbar bounds, closed the artifact UI, and closed the process.
- Not yet verified:
  - Manual UI mark loop.
...
- 2026-05-15 / capture overlay reopen iteration - reset overlay selection/hover state, position, and mask region before showing a new capture session; added an overlay reopen window regression and process-level capture drag-select/toolbar smoke.
