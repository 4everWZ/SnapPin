...
| Process UI smoke gate | CTest `ui_capture_smoke` starts `snappin.exe`, sends the capture command through `SnapPinHiddenWindow`, verifies `SnapPinOverlay` is visible with valid bounds, drag-selects a region, verifies compact `SnapPinToolbar` bounds, creates a real `SnapPinPinWindow` through toolbar `Pin`, closes the pin, then closes the process | Passed for current slice | Smoke covers baseline capture selection, toolbar visibility, and artifact-to-pin creation only; it does not replace manual mark, pin interaction, or OCR workflow verification |
...
- Verified:
  ...
  - Process UI smoke: `ui_capture_smoke` launched `snappin.exe`, opened the capture overlay through the hidden main window command path, verified visible overlay bounds, drag-selected a region, verified compact toolbar bounds, created a real pin window through toolbar `Pin`, closed the pin, and closed the process.
- Not yet verified:
  - Manual UI mark loop.
...
- 2026-05-15 / capture overlay reopen iteration - reset overlay selection/hover state, position, and mask region before showing a new capture session; added an overlay reopen window regression and process-level capture drag-select/toolbar/pin smoke.
