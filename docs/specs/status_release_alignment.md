...
| Process UI smoke gate | CTest `ui_capture_smoke` starts `snappin.exe`, sends the capture command through `SnapPinHiddenWindow`, verifies `SnapPinOverlay` is visible with valid bounds, exits it with `Esc`, then closes the process | Passed for current slice | Smoke covers capture overlay open/dismiss only; it does not replace manual selection, mark, pin, or OCR workflow verification |
...
- Verified:
  ...
  - Process UI smoke: `ui_capture_smoke` launched `snappin.exe`, opened the capture overlay through the hidden main window command path, verified visible overlay bounds, dismissed with `Esc`, and closed the process.
- Not yet verified:
  - Manual UI mark loop.
...
- 2026-05-15 / capture overlay reopen iteration - reset overlay selection/hover state, position, and mask region before showing a new capture session; added an overlay reopen window regression and process-level capture overlay smoke.
