...
| Pin image workflow and focused actions | `pin/base-use`, `pin/image` | Implemented (pending user verification) | Baseline image pin and focused actions are present; context-menu labels and image-only OCR exposure are covered | Maintain focused action/context checks |
