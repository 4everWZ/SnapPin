...
- Mark parity:
  - Current tools are limited to `Select`, `Rect`, `Ellipse`, `Line`, basic multi-click/editable `Polyline`, `Arrow`, `Serial`, basic rectangular `Mosaic` with wheel strength, basic rectangular `Blur` with wheel strength, basic whole-object/path-segment `Eraser`, basic freehand `Highlighter` with wheel-controlled opacity/strength, basic rectangular `Spotlight` with wheel-controlled dim strength, basic rectangular `Watermark` with direct text entry and wheel-controlled opacity/strength, basic rectangular `Magnifier` with wheel-controlled zoom, `Pencil`, and `Text` with a background fill toggle.
